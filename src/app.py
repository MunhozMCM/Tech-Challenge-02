"""Streamlit demo app: loads the registered model and scores test sessions."""
...
st.markdown(
    "Este aplicativo carrega o modelo treinado diretamente do **MLflow Model Registry** "
    "e realiza predições em usuários de teste."
)
...
def load_model() -> RandomForestClassifier | None:
    """Carrega a última versão do modelo registrado no MLflow Model Registry."""
    try:
...
if model:
    ...
    st.write(
        "Para demonstrar, vamos sortear um cliente aleatório da base de testes "
        "(já preprocessada) e prever se ele finalizará a compra."
    )
    ...
    st.info(
        "Execute o pipeline do DVC (`uv run dvc repro`) para treinar e registrar "
        "o modelo antes de usar o app."
    )
